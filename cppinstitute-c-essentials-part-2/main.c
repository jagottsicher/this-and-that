...
//    // i is the returnvalue of fprintf
//    // i is something, so the test if i == EOF is zero
//    printf("%d", i == EOF);
//    return 0;
//}
// Output is Hello0 in case stderr oiints to stdout, else it is 0 only
...
//
//    // i = 1
//    // *f is a pointer to a function receiving pointer to an address returning an int
//    // the adress of i is handled as arguemnt to the function call
//    // in the function value of that number is raised by one and returned as an int
//    // even the i in the function was different from the one in main, it retrieves value 2
//
//    printf("%d", i);
//    return 0;
//}
// TAKEAWAY:
// don't get confused with scopes, pointers to int and functions
// 2
...
//int main(void) {
//    FILE *f = fopen("file", "w");
//    // i received return code whatever (success)
//    int i = fputs("Hello!", f);
//    // in case of an error i was EOF
//    printf("%d", i != EOF); // prints 1 because i is not EOF
//    return 0;
//}
// 1
...
//    return 0;
//}
//// 1 if stdio.h is included, else compilation error!!!
...
//    // writing 12A to a file
//    FILE *f = fopen("file", "w");
//    int i = fputs("12A", f);
//    fclose(f);
//    // reading 19 elements from the file into a char array (string)
//    f = fopen("file", "r");
//    fgets(s, 20, f);
//    puts(s);
//    // double check char by char
//    // BE aware that fputs added a '\0' to the end of the file, or did fgets?
//    for (i=0; i < 20; i++)
//        printf("%c ",s[i]);
//
//    fclose(f);
...
//int main(void) {
//    char s[20];
//    // preparing a file handler pointer f
//    FILE *f = fopen("data", "w");
//    // i receiving success message after writing 1248 to file
//    int i = fputs("1248", f);
//    fclose(f);
//    // closing f
//
//    // opensing f handler for reading
//    f = fopen("data", "r");
//    // this is the point:
//    // to the empty array of chars (so to say a string) 124 (one less than 4 chars) are read
//    fgets(s + 2, 4, f);
//    // the first two members of the array have been skipped so we have to get along with an array looking like
//    // ??124 counting to element number 4 (from zero on):
//    // 01234
//    printf("%s\n",s);
//    // we output it
//    putchar(s[4]);
...
//
//struct S {
//    // *S is a pointer to char (here a string)
//    char *S;
//};
//
//int main(void) {
...
//    float x = 3.0, y = 2.0;
//    int i = 1, j = 2;
//    // here NOT the result is casted to int, but only x means 3 / 2.0, means the result will be a float
//    x = (int)x; // 3
//    printf("x as float %f, x as int %d\n", x, x);
//    printf("y as float %f, y as int %d\n", y, y);
//    x /= y; // 3 / 2.0 = 1.5
//    printf("x as float %f, x as int %d\n", x, x);
//    printf("y as float %f, y as int %d\n", y, y);
//    // in this part the i in casted to float, if one is a float result also is a float, x is already anway
//    printf("i as float %f, i as int %d\n", i, i);
//    printf("j as float %f, j as int %d\n", j, j);
//    printf("x as float %f, x as int %d\n", x, x);
//    printf("y as float %f, y as int %d\n", y, y);
//    printf("(float)i / j as float %f, (float)i / j as int %d\n", (float)i / j, (float)i / j);
//
//    x+= (float)i / j; // 1.5 + (float)1 / 2 = 1.5 + 0.5
//    printf("i as float %f, i as int %d\n", i, i);
//    printf("j as float %f, j as int %d\n", j, j);
//    printf("x as float %f, x as int %d\n", x, x);
//    printf("y as float %f, y as int %d\n", y, y);
//    // so the secret is: 1.50000 + 0.50000
//    printf("%f", x);
//    return 0;
//}
//// 2 (1.500000 + 0.50000)
...
//#include <stdio.h>
//
//// struct's scope is gloabally
//struct S {
//    int S[2];
//};
//
//// this struct here is locally only, means the functions scope does not change the global struct S and it's integer arrays,
//// but changes here a local copy. It returns nothing (and involking the function does not ask for). So here we are not working with
//// pointers and change globally: nothing at all!
//void f(struct S S) {
//    S.S[0] = S.S[1] + 4;
//    printf("%d, %d\n",S.S[0], S.S[1]);
//}
...
//int main(void) {
//    char *p = "\0\4\1\3\2";
//    printf("p[1] = %d   p[p[1]] = %d    *(p + 1) = %d    p[4] = %d\n", p[1] ,p[p[1]], *(p + 1), p[4]);
//    // 2 + 4 + 8
//    printf("%d\n", p[p[1]] + *(p + 1) + p[4]);
//    return 0;
//}
...
//      	j--;
//    }
//    // after test once more decremented, so i=-1 , j = 2
//    printf("i=%d, j=%d\n", i, j);
//
//    printf("%d", i + j);
//    return 0;
